...
export default createRoute(async (c) => {
  const currentLang = c.get('lang');
  const slug = c.req.param('slug');
  const detailData = await fetchMovieDetail(currentLang, slug);

  if (!detailData || !detailData.movie) {
    return c.render(<div class="text-center mt-32 text-white">Data tidak ditemukan</div>);
  }
  ...
  return c.render(
    <div>
        ...
        <div class="relative z-10 max-w-5xl w-full flex flex-col md:flex-row gap-8 items-center md:items-start mt-8">
          ...
          <div class="text-center md:text-left flex-1">
            ...
            <div class="flex items-center justify-center md:justify-start gap-3 text-sm text-gray-400 mb-6">
              <span class="text-green-500 font-bold">Cocok untukmu</span>
              <span>• {totalEpisodes} Episode</span>
            </div>
            <p class="text-gray-300 text-sm md:text-base leading-relaxed mb-8">{movie.description}</p>
            {/* Navigasi pemutaran menyertakan struktur bahasa dinamis */}
            <a href={`/episode/${currentLang}/${movie.slug}/1`} class="inline-flex w-full md:w-auto items-center justify-center bg-red-600 text-white font-bold px-10 py-4 rounded-md hover:bg-red-700 transition gap-2 shadow-lg shadow-red-900/40">
               ▶ Tonton Episode 1
            </a>
          </div>
        </div>
      ...
      <div class="max-w-5xl mx-auto px-4 py-8">
        <h3 class="text-xl font-bold text-white mb-6">Pilih Episode</h3>
        <div class="grid grid-cols-4 sm:grid-cols-6 md:grid-cols-10 gap-3">
          {episodes.map(ep => (
            <a href={`/episode/${currentLang}/${movie.slug}/${ep}`} class="bg-[#141414] border border-white/5 hover:border-white/20 hover:bg-[#262626] text-gray-300 text-center py-4 rounded-md font-medium text-sm transition-all shadow-sm">
              {ep}
            </a>
          ))}
        </div>
