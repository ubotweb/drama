...
export default createRoute(async (c) => {
  ...
  // Perlindungan ketat: pastikan heroMovie tidak error jika API membalas array kosong
  const heroMovie = movies.length > 0 ? movies[Math.floor(Math.random() * movies.length)] : null;

  return c.render(
    <div>
      {/* Tampilkan Hero Banner JIKA ada film yang dimuat */}
      {heroMovie ? (
        <div class="relative w-full h-[60vh] md:h-[75vh] flex items-end">
          ...
          <div class="relative z-10 w-full max-w-7xl mx-auto px-4 pb-12">
            ...
            <a href={`/detail/${currentLang}/${heroMovie.slug}`} class="inline-flex items-center justify-center bg-white text-black font-bold px-8 py-3 rounded-md hover:bg-gray-200 transition gap-2">
              <span class="text-xl">▶</span> {t(currentLang, 'watch_now')}
            </a>
          </div>
        </div>
      ) : (
        // Pelindung UI (Fallback): Tampil saat web baru pertama diload atau API kosong
      ...
      <div class="max-w-7xl mx-auto px-4 py-8">
        ...
        {movies.length > 0 ? (
          <div class="flex overflow-x-auto gap-4 pb-6 hide-scrollbar snap-x">
            {movies.map((movie) => (
              <a href={`/detail/${currentLang}/${movie.slug}`} class="snap-start shrink-0 w-36 md:w-48 group relative block aspect-[2/3] rounded-md overflow-hidden bg-[#141414] transition-transform duration-300 hover:scale-105">
